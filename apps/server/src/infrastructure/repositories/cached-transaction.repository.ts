...
export class CachedTransactionRepository implements ITransactionRepository {
  ...
  async findByAccountAndMonth(accountId: string, year: number, month: number): Promise<Transaction[]> {
    const cacheKey = `transactions:${accountId}:${year}:${month}`
    let transactions = cache.get<Transaction[]>(cacheKey)
    
    if (!transactions) {
      transactions = await this.repository.findByAccountAndMonth(accountId, year, month)
      cache.set(cacheKey, transactions, this.cacheTTL)
    }
  ...
  async findByAccountAndDateRange(accountId: string, startDate: Date, endDate: Date): Promise<Transaction[]> {
    return this.repository.findByAccountAndDateRange(accountId, startDate, endDate)
  }
  ...
  async getMonthlyTotals(accountId: string, year: number, month: number): Promise<{ income: number; expenses: number }> {
