...
export class CachedInvestmentRepository implements IInvestmentRepository {
  ...
  async findByAccountAndType(accountId: string, typeId: string): Promise<Investment[]> {
  ...
  async getAccountAllocation(accountId: string): Promise<Array<{ type_name: string; total_amount: number; count: number }>> {
    const cacheKey = `investment-allocation:${accountId}`
    let allocation = cache.get<Array<{ type_name: string; total_amount: number; count: number }>>(cacheKey)
