...
export class CachedMonthlyBalanceRepository implements IMonthlyBalanceRepository {
  ...
  async findByAccountAndMonth(accountId: string, year: number, month: number): Promise<MonthlyBalance | null> {
    const cacheKey = `monthly-balance:${accountId}:${year}:${month}`
    let balance = cache.get<MonthlyBalance>(cacheKey)
    
    if (!balance) {
      balance = await this.repository.findByAccountAndMonth(accountId, year, month)
      if (balance) {
  ...
  async findByAccountId(accountId: string, limit?: number): Promise<MonthlyBalance[]> {
  ...
  async findByAccountAndYear(accountId: string, year: number): Promise<MonthlyBalance[]> {
  ...
  async getYearlyTotals(accountId: string, year: number): Promise<{ totalIncome: number; totalExpenses: number; netBalance: number; totalInvestments: number }> {
    const cacheKey = `yearly-totals:${accountId}:${year}`
    let totals = cache.get<{ totalIncome: number; totalExpenses: number; netBalance: number; totalInvestments: number }>(cacheKey)
