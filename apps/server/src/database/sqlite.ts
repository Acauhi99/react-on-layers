...
class DatabaseWrapper {
  ...
  private createIndexes() {
    ...
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_categories_account ON categories(account_id)')
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_transactions_account ON transactions(account_id)')
    ...
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_investments_account ON investments(account_id)')
    ...
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_monthly_balances_account ON monthly_balances(account_id)')
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_monthly_balances_period ON monthly_balances(year, month)')
  }

  private migrate() {
    // Check if we need to migrate from old schema
    const hasAccountsTable = this.db.prepare(`
    ...
    const hasAccountIdInCategories = this.db.prepare(`
